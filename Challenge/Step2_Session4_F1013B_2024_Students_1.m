...
% figure out what the next code is for!
 ...
 %dx=h*rand()*rad;       %Think deeply in terms of dielectrophoresis!!!

 %What is happening here, complete the missing text!
 %fprintf('Erythrocyte: %d has an associated ??????? of : %f\n', ery, dx);
 
 
% (2) Inside the while that will run from the upmost Y-coordinate of the erythrocyte, say "ye", to the bottom-most (down-most)  
% Y-coordinate of the erythrocyte (say "ye>ymin"), add the following code
% for and comment what it is for. Additionally, initialize a variable "Fx",
% to calculate the x-component of the electric Colulombian force
% experienced by the blood cell.

 % What' this for?
         %addpoints(path,xe,ye); 
         %head=scatter(xe,ye, 100,'filled','o','red');   % Recall F1004B (volcano) and F1005B (F1 car) classes? 
         %drawnow  
...
% (3)Finally, also inside the while loop, but after opening and closing the for loop that will run from charge 1 all the way to the last 
% charge in the plates (electrodes), add the following code, and ad before, comment, comment, comment!

% What is this for?
         %if ye>ymin+dx
         %    delete(head);    % Recall F1004B (volcano) and F1005B (F1 car) classes?
         %end
...
% Open a for loop that will run over the total number of erythrocytes (Ne) defined by the user, 
% and within it initialize the erythrocyte position (xe, ye) and velocities (Vx , Vy)... Think about it! :)

% Next, inside this loop and after the initialization referred above, use (define) a while that will 
...
% Finally, inside this while open another for loop that will run from charge 1 all the way to the last 
...
% Loop over each erythrocyte
for ery=1:?
    

    % --Initialize the erythrocyte position and velocities--
     xe=?; ye=?;    % Erythrocyte initial position
     Vy=?; Vx=?;    % Initial velocities
      
     % Loop until the erythrocyte reaches the bottom of the workspace
     while ? > ?

 
         % Loop over charges on plates (electrodes), to calculate total force on erythrocyte.
